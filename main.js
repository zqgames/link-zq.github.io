...
function LinkGame(config) {
  ...
  this.cellWidth = config.cellWidth || 40; // 每格的的宽度
  this.cellHeight = config.cellHeight || 40; // 每格的高度
  this.cols = config.cols + 2 || 7; // 列数
  this.rows = config.rows + 2 || 10; // 行数
...
LinkGame.prototype = {
  ...
  drawLine: function (callback) {
    ...
    for (var i = 0; i < linkList.length; i++) {
      var x = linkList[i].col === 0 ? 0 : (linkList[i].col === this.cols - 1 ? $('#game').width() : linkList[i].col * 50 - 25);
      var y = linkList[i].row === 0 ? 0 : (linkList[i].row === this.rows - 1 ? $('#game').height() : linkList[i].row * 50 - 25);
      coordinate.push([x, y]);
    }
    var ctx = $canvas[0].getContext('2d');
    ctx.beginPath();
    ctx.strokeStyle = "#EADEE3";
    ctx.fillStyle = "#EADEE3";
    ctx.lineWidth = 0.01;
    ctx.save();
    for (var i = 0; i < linkList.length; i++) {
      if (i === 0) {
